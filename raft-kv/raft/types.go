...
type LogEntry struct {
	...
	Command CommandType // The command type (Put, Append, Get).
...
// SubmitCommandReply is the RPC reply when a client submits a command.
type SubmitCommandReply struct {
	Index    int  // Log index where the command was appended.
	Term     int  // Term of the command.
	IsLeader bool // Whether the node handling the request is the leader.
}

// Constants for Raft configuration.
const (
	ElectionTimeoutMin = 300 * time.Millisecond // Increased from 150ms to 300ms
	ElectionTimeoutMax = 600 * time.Millisecond // Increased from 300ms to 600ms
	HeartbeatInterval  = 100 * time.Millisecond // Increased from 50ms to 100ms
)
