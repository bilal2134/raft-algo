...
// applyEntries applies all committed but not yet applied log entries to the state machine.
func (rn *RaftNode) applyEntries() {
	for rn.lastApplied < rn.commitIndex {
		...
		// In a full system, you would send this to the state machine (e.g., via applyCh).
		// For example: result := applyCommand(entry) and then rn.applyCh <- result
	}
}
